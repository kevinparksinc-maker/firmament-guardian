    ...
    <div className="min-h-screen bg-[#07101d] text-slate-200">
      ...
      <div className="relative mx-auto flex min-h-screen max-w-[1600px]">
        ...
        <main className="min-w-0 flex-1 px-5 py-5 sm:px-8 lg:px-10 lg:py-8">
          <header className="flex flex-col gap-5 border-b border-white/[0.07] pb-7 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3 lg:hidden"><div className="brand-mark"><Orbit size={18} /></div><span className="font-display text-sm text-white">Firmament / Lab</span></div><div><div className="flex items-center gap-2"><span className="status-dot" /><p className="text-[10px] font-bold uppercase tracking-[0.25em] text-cyan-300/80">Research environment · shell v0.1</p></div><h1 className="mt-3 font-display text-3xl tracking-[-0.03em] text-white sm:text-4xl">Simulation control room</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Replay verified historical events through one consistent chart context. The Lab shows the chart, compares both frames, and explains the result layer by layer.</p></div><div className="flex items-center gap-2"><button onClick={() => comingSoon("Documentation")} className="icon-button" aria-label="Documentation"><CircleHelp size={17} /></button><button onClick={() => setShowConfig(!showConfig)} className={`button-secondary ${showConfig ? "button-secondary-active" : ""}`}><Settings2 size={15} /> Run configuration</button><button onClick={() => comingSoon("Export report")} className="icon-button" aria-label="Export report"><Download size={17} /></button></div></header>

          <section className="mt-6 rounded-2xl border border-cyan-300/15 bg-gradient-to-r from-cyan-300/[0.08] via-white/[0.025] to-violet-300/[0.06] p-5 shadow-[0_18px_60px_rgba(0,0,0,.18)]"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div><p className="eyebrow text-cyan-200/80">Start here</p><h2 className="mt-2 font-display text-xl text-white">Run your first historical replay</h2><p className="mt-1 max-w-2xl text-xs leading-5 text-slate-400">Bring a verified CSV, let the Lab check each row, then run the same Firmament calculations across every accepted event.</p></div><button onClick={() => fileInputRef.current?.click()} className="button-primary shrink-0"><Upload size={15} /> Upload a CSV</button></div><div className="mt-5 grid gap-3 md:grid-cols-3"><div className="guide-step"><span>01</span><div><strong>Import data</strong><p>Upload one row per game with date, time, location, teams, sport, and winner.</p></div></div><div className="guide-step"><span>02</span><div><strong>Run the engine</strong><p>The Lab builds the chart and evaluates God View plus AgentView.</p></div></div><div className="guide-step"><span>03</span><div><strong>Read the audit</strong><p>See placements, house assignments, and which layers hit or missed.</p></div></div></div></section>
          ...
          <section className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.42fr)_minmax(360px,.9fr)]">
            <div className="panel"><div className="panel-header"><div><p className="eyebrow">03 / Run it</p><h2 className="section-title">Start a historical replay</h2></div><Pill tone="emerald">Engine connected</Pill></div><div className="grid gap-4 px-5 pb-5 md:grid-cols-[1fr_1fr_auto]"><div className="runner-cell"><div className="runner-icon"><CalendarDays size={16} /></div><div><p className="runner-label">Selected dataset</p><p className="runner-value">{importedDataset?.name ?? templates.find((t) => t.id === activeTemplate)?.label}</p><p className="runner-meta">{importedDataset ? `${importedDataset.validRowCount.toLocaleString()} valid / ${importedDataset.rowCount.toLocaleString()} rows · dataset v${importedDataset.id}` : `${templates.find((t) => t.id === activeTemplate)?.events.toLocaleString()} staged events · import CSV to run real data`}</p></div></div><div className="runner-cell"><div className="runner-icon"><Layers3 size={16} /></div><div><p className="runner-label">Calculation frames</p><p className="runner-value">God View + AgentView</p><p className="runner-meta">God View = fixed background · AgentView = local horizon</p></div></div><div className="flex items-center"><button onClick={runSimulation} disabled={isRunning || !importedDataset && simulateEvent.isPending} className="button-run"><Play size={15} fill="currentColor" />{isRunning ? `${activeRunId ? `Running ${runProgress}%` : `Preparing ${runProgress}%`}` : importedDataset ? "Run batch" : "Run fixture"}</button></div></div>{isRunning && <div className="mx-5 mb-3 h-1 overflow-hidden rounded-full bg-white/[0.07]"><div className="h-full rounded-full bg-gradient-to-r from-cyan-300 to-violet-300 transition-all duration-300" style={{ width: `${runProgress}%` }} /></div>}{activeRunId && <div className="mx-5 mb-5 flex items-center justify-between text-[10px] uppercase tracking-[0.16em] text-slate-600"><span>Run {activeRunId} · {runStatus.data?.status ?? "starting"}</span><span>{runStatus.data ? `${runStatus.data.completedEvents + runStatus.data.failedEvents}/${runStatus.data.totalEvents} processed` : "Initializing"}</span></div>}<div className="mx-5 mb-5 flex flex-wrap items-center gap-x-5 gap-y-2 border-t border-white/[0.07] pt-4 text-[11px] text-slate-500"><span className="flex items-center gap-2"><Clock3 size={13} /> Historical replay only</span><span className="flex items-center gap-2"><Radio size={13} /> No live promotion</span><span className="flex items-center gap-2"><Target size={13} /> Outcome comparison persisted</span></div></div>
