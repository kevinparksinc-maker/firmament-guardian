      ...
      <div className="grid gap-0 lg:grid-cols-[minmax(0,1fr)_330px]">
        <div className="p-5 sm:p-7">
          <div className="mb-4 flex items-center gap-3">
            ...
            <div><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.24em] text-cyan-200"><Sparkles className="h-3.5 w-3.5" /> Firmament Host</div><p className="mt-1 text-xs text-slate-400">Your guide to using the observatory</p></div>
          </div>
          <div className="h-64 space-y-3 overflow-y-auto rounded-2xl border border-white/10 bg-black/20 p-4" aria-live="polite">
            {messages.map((message, index) => <div key={`${message.role}-${index}`} className={`flex items-start gap-2.5 ${message.role === "user" ? "justify-end" : ""}`}>
              ...
              <div className={`max-w-[85%] rounded-2xl px-3.5 py-2.5 text-sm leading-6 ${message.role === "user" ? "bg-violet-400/20 text-violet-50" : "bg-white/[0.07] text-slate-200"}`}>{message.role === "assistant" ? <Streamdown>{message.content}</Streamdown> : message.content}</div>
